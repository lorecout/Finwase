...
class FirebaseService {
  ...
  Future<UserCredential?> signInWithGoogle() async {
    try {
      ...
      if (result.user != null) {
        debugPrint(
            '✅ FIREBASE: Google Sign-In sucesso - UID: ${result.user?.uid ?? 'N/A'}');
        if (result.user != null) {
      ...
      debugPrint(
          '❌ FIREBASE: Erro ao fazer signInWithGoogle: ${e.code} - ${e.message}');
      rethrow;
    } catch (e) {
    ...
    try {
      ...
      if (isSignedInWithGoogle) {
        ...
        if (googleUserInfo.displayName != null &&
            (googleUserInfo.displayName?.isNotEmpty ?? false)) {
