...
class AppState extends ChangeNotifier {
  // Salva uma transação no Firestore, associada ao usuário autenticado
  Future<void> salvarTransacao(TransactionModel transacao) async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) {
      debugPrint('❌ Não é possível salvar: usuário não autenticado');
      return;
    }
    try {
      await FirebaseFirestore.instance
          ...
          .doc(transacao.id)
          .set(transacao.toMap());
      debugPrint('✅ Transação salva para o usuário ${user.uid}');
      // Atualiza localmente
      final idx = _transacoes.indexWhere((t) => t.id == transacao.id);
      if (idx >= 0) {
        _transacoes[idx] = transacao;
      } else {
